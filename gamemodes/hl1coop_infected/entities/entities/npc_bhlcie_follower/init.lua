	...
	if key == "follower_rumble" then
		...
		if IsValid(self.dummyEnt) then
			...
			timer.Simple(2.5,function() if IsValid(self.dummyEnt) then
				...
				if self.BHLCIE_Follower_Difficulty == 3 then
					util.VJ_SphereDamage(self, self, self.dummyEnt:GetPos(), 175, 40, DMG_CRUSH, true, true, {DisableVisibilityCheck=true, Force=8110, UpForce=8110})
				elseif self.BHLCIE_Follower_Difficulty == 2 then
					util.VJ_SphereDamage(self, self, self.dummyEnt:GetPos(), 175, 30, DMG_CRUSH, true, true, {DisableVisibilityCheck=true, Force=8110, UpForce=8110})
				else
					util.VJ_SphereDamage(self, self, self.dummyEnt:GetPos(), 175, 20, DMG_CRUSH, true, true, {DisableVisibilityCheck=true, Force=8110, UpForce=8110})
				end
...
---------------------------------------------------------------------------------------------------------------------------------------------
function ENT:CustomOnDeath_BeforeCorpseSpawned(dmginfo, hitgroup) -- just incase this happens somehow
	if IsValid(self.dummyEnt) then
